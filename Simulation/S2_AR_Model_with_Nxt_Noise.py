"""
Python implementation of Simulation 2 of the T-GIC method for AR model with Nxt noise.
"""
import numpy as np
...
def generate_ar_data(sample_num, ar_params, true_k, true_alpha, true_s, true_constant, non_gaussian=False,
                     outliers=False):
    ar = np.r_[1, -np.array(ar_params)]
    ma = np.r_[1]
    all_num = sample_num + drop_num
    if non_gaussian:
        noise = generate_noise_data(true_k, true_alpha, true_s, all_num)
    else:
        noise = np.random.normal(scale=true_k, size=all_num)
    ...
    data0 = ar_process.generate_sample(nsample=all_num, scale=1.0, distrvs=lambda size: noise) + true_constant
    final_data = data0[drop_num:]
    return final_data
...
def gradient_descent(sample, order, max_order, lr=0.04, epochs=500):
    np.random.seed(1000)
...
for i in sample_sizes:
    ...
    for j in range(num_simulations):
        idx = 0 + j
        np.random.seed(idx)
        data = generate_ar_data(i, ar_params=[0.5, -0.25, 0.1], true_k=1.5, true_alpha=0.5, true_s=0.5,
                                true_constant=3.0, non_gaussian=True)
...
with open('./S2_AR_Model_with_Nxt_Noise_result.json', 'w') as w:
